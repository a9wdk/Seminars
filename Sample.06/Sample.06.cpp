//*****************************************************************************
// (v06) Использование хэш-функций для контроля целостности кода и данных.
            ...
            for (x = 5; x < ((rect.right - rect.left)/2 - xGap); x += xGap) {
                ...
                if(z1 == z0 && z2 == z0) {
                    ...
                    if(!CheckCRC32(&F1_CRC)) 
                        MsgBox("Data CRC Error", MB_SYSTEMMODAL | MB_ICONERROR, 
                               "Original CRC\t%08Xh\nCurrent CRC\t%08Xh\n", F1_CRC.OrgCrc, F1_CRC.CurrCrc);
...
void Function2(PVOID hwnd) {
    ...
    l1: // Начало участка кода, защищенного CRC.
    ...
    l2: // Конец участка кода, защищенного CRC.
            ...
            for (x = (rect.right - rect.left)/2; x < rect.right - rect.left - xGap; x += xGap) {
                ...
                if(z1 == z0-1 && z2 == z1-1) {
                    ...
                    if(!CheckCRC32(&Fn2_CRC)) 
                        MsgBox("Code CRC Error", MB_SYSTEMMODAL | MB_ICONERROR, 
                               "Start Address\t%Xh\nCode Length\t%d(%Xh)\nOriginal CRC\t%08Xh\nCurrent CRC\t%08Xh",
                               Fn2_CRC.Addr, Fn2_CRC.Length, Fn2_CRC.Length, Fn2_CRC.OrgCrc, Fn2_CRC.CurrCrc);
