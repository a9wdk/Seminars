//*****************************************************************************
// (v03) Защита потока фоновой проверки ключа от принудительного завершения
// или незапуска. Маскировка характерных сигнатур.
//
...
void KeyBackgroundChk(PVOID hwnd) {
	...
	while(TRUE) {
		...
		if(status != HASP_STATUS_OK) {		
			...
			if(hMsgState == 0) hMsg = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)KeyBackgroundChkMsg, (PVOID)status, 0, NULL);
		} else {							
			// Ключ обнаружен.
			HaspKeyPresent = TRUE;
...
void CALLBACK KBChkProtect(HWND hwnd, UINT msg, UINT idTimer, DWORD dwTime) {
	...
	if(Noise != HaspKeyPresent) {		// Поток KeyBacgroundChk работает штатно. Рандомизируем таймаут реакции.
		...
		sprintf_s(tmp, sizeof(tmp), "%s (новый таймаут: %d сек.)", szTitle, ProtectTimeout);
	} else {							// Поток KeyBacgroundChk не работает.
		MessageBeep(MB_ICONHAND);
		if(ProtectTimeout) {			// Если таймаут не истек - уменьшаем его на единицу.
			ProtectTimeout -= 1;	
			sprintf_s(tmp, sizeof(tmp), "%s (осталось: %d сек.)", szTitle, ProtectTimeout);
