//*****************************************************************************
// (v08) Пример реализации самовосстанавливающейся высокоэшелонированной
// защиты, использующей для работы механизм TLS_Callbacks.
// Для данного примера в демонстрационных целях были написаны два патча (см.
// каталог Test), поэтому пересобирать пример не рекомендуется, т.к. патчи
// после этого могут не подойти к примеру.
//
// После сборки исполняемый файл примера нужно обработать вспомогательными утилитами:
// ..\Post-Build Utilites\DataEncrypt
// ..\Post-Build Utilites\CRC32Protect
// ..\Post-Build Utilites\ECCProtect с параметром /tls
//
...
EccDsc  WM_Ecc = {(PBYTE)0x78563412};       // ECC-дескриптор участка кода между метками l1 и l2.
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    AsmEccCodeMrk(WM_Ecc, l1, l2, WM_Ecc_Length, 0);    // ECC-маркер участка кода между метками l1 и l2.
}
...
EccDsc  WP_Ecc = {(PBYTE)0x78563412};       // ECC-дескриптор участка кода между метками l1 и l2.
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    AsmEccCodeMrk(WP_Ecc, l1, l2, WP_Ecc_Length, 1);    // ECC-маркер участка кода между метками l1 и l2.
}
...
#define Fn1_Ecc_Length  128                     // Размер ECC для защищенного участка кода.
...
#define Fn2_Ecc_Length  128                     // Размер ECC для защищенного региона кода.

HaspSession Fn2  = {HASP_INVALID_HANDLE_VALUE, HASP_HASP_NOT_FOUND, F2_Feature};

CrcDsc F2_Crc  = {(PBYTE)0x78563412};           // CRC-дескриптор массива F2.
...
CrcDataMrk    F2_Mrk2 = {CrcDataSig, &F2_Crc, &F2, sizeof(F2), 0};                  // CRC-маркер массива F2
EccDataMrk    F2_Mrk3 = {EccDataSig, &F2_Ecc, &F2, sizeof(F2), F2_Ecc_Length, 4};   // ECC-маркер массива F2
...
void Function2(PVOID hwnd) {
    ...
    AsmEccCodeMrk(Fn2_Ecc, l1, l2, Fn2_Ecc_Length, 5);      // ECC-маркер участка кода между метками l1 и l2.
}
...
EccDsc KBC_Ecc = {(PBYTE)0x78563412};       // ECC-дескриптор участка кода между метками l1 и l2.
...
void KeyBackgroundChk(PVOID hwnd) {
    ...
    AsmEccCodeMrk(KBC_Ecc, l1, l2, KBC_Ecc_Length, 6);      // ECC-маркер участка кода между метками l1 и l2.
}
...
EccDsc KBCP_Ecc = {(PBYTE)0x78563412};      // ECC-дескриптор участка кода между метками l1 и l2.
...
//          SetThreadPriority(hKeyBackgroundChk, THREAD_PRIORITY_LOWEST);
    ...
    // Маркер, описывающий характеристики региона кода, помеченного метками l1 и l2.
    AsmEccCodeMrk(KBCP_Ecc, l1, l2, KBCP_Ecc_Length, 7);        // ECC-маркер участка кода между метками l1 и l2.
}
...
///////////////////////////////////////////////////////////////////////////////////////////////////
//
//  Весь нижерасположенный код получает управление только через механизм TLS_Callback
//
///////////////////////////////////////////////////////////////////////////////////////////////////

// Создаем вручную структуру TLS_Directory:
...
TLS TLS_Addr = {TLSDirSign, &TlS_Directory};        // Передача адреса TLS_Directory во внешнюю утилиту.

///////////////////////////////////////////////////////////////////////////////////////////////////
// Возможные значения параметра Reason (приведены для DLL):
// 1 (DLL_PROCESS_ATTACH) - действия, связанные с присоединением к адресному пространству вызывающего процесса
// 2 (DLL_THREAD_ATTACH)  - действия, связанные с появлением в области видимости еще одного дочернего потока
// 3 (DLL_THREAD_DETACH)  - действия, связанные с выходом дочернего потока из области видимости DLL
// 0 (DLL_PROCESS_DETACH) - действия, связанные с отсоединением DLL от адресного пространства вызывающего процесса

//////////////////////////////////////////////////////////////////////////////////////////////
// Первая функция TLS_Callback, пустышка. Содержит безобидный код, выполняющий инициализацию некоторых
// переменных перед стартом приложения. Предназначена для обмана всяческих plugin'ов для отладчиков,
// позволяющих остановиться на точке входа TLS_Callback-функции. Подавляющее большинство таких plugin'ов
// умеют работать только с первой TLS_Callback-функцией.
void _stdcall TlsEmpty(PVOID DllHandle, DWORD Reason, PVOID Reserved) {
    if(Reason == 1) { 
        f1Terminate = TRUE;
        f2Terminate = TRUE;
...
//////////////////////////////////////////////////////////////////////////////////////////////
// Вторая функция TLS_Callback. Перед завершением приложения проверяет статистику по состоянию
// первого эшелона защиты, собранную вторым эшелоном (CRC32). Если пороговые значения счетчиков
// достигнуты, создает файл-триггер, активирующий третий эшелон защиты.
void _stdcall TlsTriggerUp(PVOID DllHandle, DWORD Reason, PVOID Reserved) {
    HANDLE  hTriggerFile;
...
//////////////////////////////////////////////////////////////////////////////////////////////
// Третья функция TLS_Callback, собственно, третий эшелон защиты. Проверяет состояние триггера
// перед стартом приложения. Если файл-триггер существует, начинает сканирование регионов, защищенных ECC.
void _stdcall TlsCheckTrigger(PVOID DllHandle, DWORD Reason, PVOID Reserved) {
    HANDLE  hTriggerFile;
