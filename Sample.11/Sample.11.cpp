//*****************************************************************************
// (v11) Борьба с эмуляторами. Противодействие эмуляции, контроль целостности
// и восстановление кода библиотечных и иных внешних функций. 
//
...
// ..\Post-Build Utilites\ECCProtect
//
...
#define API_DEPTH       60      // На какую глубину сканировать код HASP API от точки входа
#define API_ECC_LEN     120     // Длина ECC для восстановления разрушенного кода

// Дескрипторы подлежащих контролю точек входа в LDK API
CrcDsc  login_CRC   = {(PBYTE)0x78563412};          // CRC-дескриптор функции hasp_login
EccDsc  login_ECC   = {(PBYTE)0x78563412};          // ECC-дескриптор функции hasp_login
CrcDsc  getinfo_CRC = {(PBYTE)0x78563412};          // CRC-дескриптор функции hasp_getinfo
EccDsc  getinfo_ECC = {(PBYTE)0x78563412};          // ECC-дескриптор функции hasp_getinfo
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    DWORD mbRet, loginCheck, getinfoCheck;
    ...
    // Проверяем и, при необходимости, исправляем разрушенный код LDK API
    loginCheck   = CheckCRC32(&login_CRC);
    getinfoCheck = CheckCRC32(&getinfo_CRC); 
    if(!loginCheck || !getinfoCheck) {
        mbRet =  MsgBox("LDK API Entry points CRC Check", MB_SYSTEMMODAL | MB_ICONQUESTION | MB_YESNO, 
                        "hasp_login() entry point check:   \n\n   Original CRC\t%08Xh\n   Current CRC\t%08Xh\n\n"
                        "hasp_get_info() entry point check:\n\n   Original CRC\t%08Xh\n   Current CRC\t%08Xh\n\n"
                        "Do you wish to repair a corrupted code?", 
                        login_CRC.OrgCrc, login_CRC.CurrCrc, getinfo_CRC.OrgCrc, getinfo_CRC.CurrCrc);
        if(mbRet == IDYES) RSCDataScan(EccListEntry.EntryPoint);
    }
...
void Function1(PVOID hwnd) {
    ...
    // Инициализируем указатель на переменную. 
    __asm mov z0, offset Z0;
    ...
    // Размещаем в коде переменную z0
    DWORDVarInCode(Z0);
